...
@Component({
  selector: 'app-posteventos',
  standalone: true,
  imports: [],
  templateUrl: './posteventos.component.html',
  styleUrl: './posteventos.component.css'
})
export class PosteventosComponent {
  @Output() Completado = new EventEmitter<boolean>();
  @ViewChild('cerrarBoton') cerrarBotonRef!: ElementRef;

  constructor(private api: PeticionesapiService, private matrizDatos : MatrizServicio) { }
  ...
  enviarDatos(e: any) {
    ...
    if (enviar) {
      ...
      const datos = {
        ...
        id_matriz : this.matrizDatos.getData().id
